...
export function registerBatchTools(server) {
  server.tool('batch_run', 'Run an action across multiple symbols and/or timeframes', {
    ...
    delay_ms: numberParam().optional().describe('Delay between iterations in ms (default 2000)'),
    ohlcv_count: numberParam().optional().describe('Bar count for get_ohlcv action (default 100)'),
